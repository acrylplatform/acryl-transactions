...
import { address, publicKey } from '@acryl/ts-lib-crypto'
import { MATCHER_URL, MASTER_SEED, CHAIN_ID, TIMEOUT, API_BASE, randomHexString } from './config'

const matcherUrl = MATCHER_URL
...
describe('Assets', () => {
  ...
  beforeAll(async () => {
    ...
    const mtt = massTransfer({
      transfers: [
        { recipient: address(account1, CHAIN_ID), amount: 5.1 * acrl },
        { recipient: address(account2, CHAIN_ID), amount: 1 * acrl }
      ]
    }, MASTER_SEED)
  ...
  describe('Ordinary assets', () => {
    ...
    it('Should transfer asset', async () => {
      const transferParams: ITransferParams = {
        ...
        attachment: '3JRnNWu4ubdf9PtZdSzUo4Emp4CwMRn68dF',
      }
  ...
  describe('Other', () => {
    it('Should create alias for address', async () => {
      ...
      const aliasTx = alias({ alias: aliasStr, chainId: 'K' }, account1)
    ...
    //   // GIVE ACRYL TO TEST ACC
    //   // const transferTx = transfer({ recipient: address(account2, 'K'), amount: 100000000 }, MASTER_SEED)
