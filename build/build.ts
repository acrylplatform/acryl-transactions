...
async function build() {
  try {
    ...
    await npmInstall('acryl-crypto', 'tmp')

    await copy(p('../src'), p('tmp/src'))
    await copy(p('../usage'), p('tmp/usage'))
    await copy(p('../tsconfig.json'), p('tmp/tsconfig.json'))
    await run('tsc', p('tmp'))
    await create(p('tmp/node_modules/@acryl/'))
    await copy(p('tmp/dist'), p('tmp/node_modules/@acryl/acryl-transactions'))
    await copyJson(p('../package.json'), p('tmp/node_modules/@acryl/acryl-transactions/package.json'), { main: 'index.js', types: 'index.d.ts' })
    await remove(p('tmp/dist'))
    await run('ts-node usage/index.ts', p('tmp'))
    await run('typedoc', p('tmp'))
    await run('tsc', p('tmp'))
    // const latestVersion = await npmGetVersion('@acryl/acryl-transactions')
